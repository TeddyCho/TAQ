...
$$r_{z_{i}z_{j}} = \frac{\sum_{t=1}^{n-1} z_{it} z_{j(t+1)} - n\bar{z_i}\bar{z_j}}{(n-2)s_{z_i}s_{z_j}}$$
where $z_{i,t}$ and $z_{j,t}$ are exchange volume share scores for exchange $a$ and $b$ at time $t$, and $s_{z_i}$ and $s_{z_j}$ are the sample standard deviations for $z_{it}$ and $z_{jt}$, respectively.
